...
tfidfwords <- words %>% bind_tf_idf(word, location , n) %>% arrange(desc(tf_idf))
...
zipfs_function <- function(dataframe)
{
words <- dataframe %>% unnest_tokens(word, speech)
...
#Gets tge top 1000 ranks
tf_top1000 <- ranked %>% group_by(location) %>% filter(rank(desc(tf), ties.method = "average") <= 1000)

options(scipen=999)

tf_top1000 %>% 
  ggplot(aes( x = rank , y = as.numeric(tf), color = location)) +
  geom_line(size = 0.75) +
  geom_smooth(method = "lm", se = FALSE , color = "black") +
  scale_y_log10() +
  scale_x_log10() +
  labs(y = "Term frequency (tf)", x = "Word rank", title = "Zipf's Law for Donald Trump's Rallies Data") +  theme_minimal()

}
zipfs_function(speeches)
